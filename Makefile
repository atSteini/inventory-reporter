...
FULL_BRAFTERSECTION := -1
FULL_BRAFTERTOTAL := 2
FULL_SECTIONCMDS := '\pagebreak' '\section{$${KATEGORIE}}'

GIG_SHEET := 'Gigprotokoll'
GIG_GEAR_COL := 'Ausrüstung'
GIG_NAMED_GEAR_SHEET = 'Standardinventarlisten'
GIG_IDS_KV := 'Name' 'IDs'
GIG_BREAK_AFTER_SEC := -1
GIG_BREAK_AFTER_TOTAL := 10
GIG_WRITE_CHECKLIST_NEWLINE = 1
GIG_WRITE_SECTIONS = $(ENV_GIG_WRITE_SECTIONS)
GIG_CHECKLIST := $(ENV_GIG_CHECKLIST)
GIG_ID = $(ENV_GIG_ID)
...
WRITE_GIG_SCRIPT := write-gig-list.py

MAIN_TEX_NAME_FULL := inventory_full
MAIN_TEX_NAME_GIG := inventory_gig
TEX_OPTIONS := -halt-on-error -interaction=nonstopmode
...
run_full: clean_full
	...
	python3 --version
	python3 $(PYTHON_SCRIPT) -filepath $(PATH_TO_EXCEL) -sheet $(INV_SHEET) --sorts $(SORT_BY) --writesections $(INCLUDE_CHANGE_MARKERS) --braftersection $(FULL_BRAFTERSECTION) --braftertotal $(FULL_BRAFTERTOTAL) --sectioncmds $(FULL_SECTIONCMDS)
	@printf "[\e[0;36mINFO\e[0m] Python script finished!\n"

	@printf "[\e[0;36mINFO\e[0m] Generating pdf...\n"
	cd "$(OUT_PATH_FULL)/$(OUT_PATH_MAIN_FULL)"; pdflatex $(TEX_OPTIONS) $(MAIN_TEX_NAME_FULL).tex; pdflatex $(TEX_OPTIONS) $(MAIN_TEX_NAME_FULL).tex; 
...
run_gig: clean_gig
	...
	python3 $(WRITE_GIG_SCRIPT) -filepath $(PATH_TO_EXCEL) -gigsheet $(GIG_SHEET) -outpath $(OUT_PATH_GIG)/$(OUT_PATH_IDS_GIG)/$(IDS_FILE) -gearcol $(GIG_GEAR_COL) -invsheet $(GIG_NAMED_GEAR_SHEET) --namedgearcols $(GIG_IDS_KV) --forid $(GIG_ID) --checklist $(GIG_CHECKLIST) --claddnewline $(GIG_WRITE_CHECKLIST_NEWLINE)
	@printf "[\e[0;36mINFO\e[0m] ID finder script finished!\n"

	@printf "[\e[0;36mINFO\e[0m] Executing latex generator python script...\n"
	python3 $(PYTHON_SCRIPT) -filepath $(PATH_TO_EXCEL) -sheet $(INV_SHEET) --sorts $(SORT_BY) --writesections $(GIG_WRITE_SECTIONS) --tplitem $(TPL_GIG_ITEM) --tplmain $(TPL_GIG_MAIN) --outpath $(OUT_PATH_GIG) --namemain $(MAIN_TEX_NAME_GIG).tex --braftersection $(GIG_BREAK_AFTER_SEC) --braftertotal $(GIG_BREAK_AFTER_TOTAL) --gigdatafrom $(OUT_PATH_GIG)/$(OUT_PATH_IDS_GIG)/$(IDS_FILE) --sectioncmds $(SECTION_CHANGE_GIG)
	@printf "[\e[0;36mINFO\e[0m] Latex generator script finished!\n"

	@printf "[\e[0;36mINFO\e[0m] Generating pdf...\n"
	cd "$(OUT_PATH_GIG)/$(OUT_PATH_MAIN_GIG)"; pdflatex $(TEX_OPTIONS) $(MAIN_TEX_NAME_GIG).tex; pdflatex $(TEX_OPTIONS) $(MAIN_TEX_NAME_GIG).tex; 
